...
with open('La Sportiva/urls.csv', 'r', newline='') as file:
    urls = []
    ...
    if response.status_code == 200:
        ...
        try:
            ...
            try:
                ...
                product_description = soup.find('div', class_='product-short-description').text.strip()
            try:
                product_price = soup.find('span', class_='product__price product__price--compare').text.strip()
            except:
                product_price = soup.find('span', class_='normal-price').find('meta', class_='price')['content']
